...
```bash
pip install -e ".[django]"          # or .[django,production] for GCP libs
docker compose up -d postgres redis
PYTHONPATH=. python manage.py check
PYTHONPATH=. python -m pytest tests/web tests/contract -v
# Run the backend (boots the platform, serves the Django ASGI app):
PYTHONPATH=.:a2h python -m src.bootstrap --no-auth --dashboard --port 5055
# Admin: http://localhost:5055/admin/   (manage.py createsuperuser first)
```
(`tests/contract/fastapi_routes.json` is the frozen route-contract baseline.)
...
3. **Run the worker tier.** Celery worker + beat processes; each worker must install
   ...
   `worker_process_init`):
   ```bash
   celery -A forgeos_web.celery_app worker -Q agents,agents_resume,scheduled,agents_longrun
   celery -A forgeos_web.celery_app beat --scheduler django_celery_beat.schedulers:DatabaseScheduler
   ```
