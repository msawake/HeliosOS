...
def _auth_enabled() -> bool:
    """Auth is on only if the setting allows it AND boot didn't pass --no-auth
    (the di context carries the boot's auth_enabled flag)."""
    if not getattr(settings, "FORGEOS_AUTH_ENABLED", True):
        return False
    from forgeos_web import di

    ctx = di.try_get_context()
    if ctx is not None and not ctx.auth_enabled:
        return False
    return True
