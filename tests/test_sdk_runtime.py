...
class TestPolicyChecks:
    ...
    async def test_check_data_no_boundaries_cross_namespace_denied(self):
        """Without declared boundaries, cross-namespace access is denied (default-deny)."""
        rt, *_, agent_id = _make_runtime()
        token = rt.bind(agent_id, namespace="sales")
        try:
            decision = await rt.check_data("finance")
            assert decision.denied  # security fix: default deny cross-namespace
        finally:
            rt.unbind(token)

    async def test_check_data_no_boundaries_own_namespace_allowed(self):
        """Without declared boundaries, agent can access its own namespace."""
        rt, *_, agent_id = _make_runtime()
        token = rt.bind(agent_id, namespace="sales")
        try:
            decision = await rt.check_data("sales")
            assert decision.allowed
        finally:
            rt.unbind(token)
