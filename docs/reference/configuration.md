...
The `infrastructure/docker/docker-compose.yaml` defines three services:
...
For local development, start only Postgres:
...
The `DB_PASSWORD` is auto-generated by `docker-setup.sh` and stored in `infrastructure/docker/.env`.
