...
``httpx``, and it imports nothing from the Helios OS source tree, so it runs from
...
def main() -> None:
    ...
    logger.info("Helios OS MCP Server — url=%s transport=%s", FORGEOS_URL, args.transport)
