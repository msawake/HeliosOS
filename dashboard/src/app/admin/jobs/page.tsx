...
export default function SchedulerPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-white mb-2">Scheduler</h1>
      ...
      {loading ? <p className="text-gray-400">Loading jobs...</p> : jobs.length > 0 ? (
        <div className="space-y-2">
          {jobs.map((j) => (
            <div key={j.agent_id} className="bg-gray-900 border border-gray-800 rounded-xl p-4">
              <div className="flex items-center gap-3 mb-2">
                <span className="text-white font-medium">{j.name || j.agent_id}</span>
                <span className={`text-xs px-2 py-0.5 rounded ${
                  j.status === 'active' ? 'bg-green-500/20 text-green-400' : 'bg-gray-700 text-gray-400'
                }`}>{j.status}</span>
              </div>
              <div className="grid grid-cols-3 gap-4 text-xs text-gray-500">
                ...
                <div>
                  ...
                  <p className="text-gray-300">{j.next_run ? new Date(j.next_run).toLocaleString() : 'N/A'}</p>
                </div>
                <div>
                  <p className="text-gray-600 mb-0.5">Last Run</p>
                  <p className="text-gray-300">{j.last_run ? new Date(j.last_run).toLocaleString() : 'Never'}</p>
                </div>
              </div>
            </div>
          ))}
