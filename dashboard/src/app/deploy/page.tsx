...
export default function DeployPage() {
  ...
  const deploy = async () => {
    setDeployError(null);
    // Parse once for a fast local check before the server's authoritative one.
    try {
      yaml.load(text);
    } catch (e) {
      setParseError(e instanceof Error ? e.message : 'Invalid YAML');
  ...
  return (
    <div>
      <PageHeader
        title="Deploy"
        description="Paste an agent manifest, validate it, and deploy. Mirrors forgeos deploy."
