...
export default function AgentDetailPage() {
  ...
  useEffect(() => {
    if (!agentId) return;
    fetch(`/api/platform/agents/${agentId}`, { headers: getAuthHeaders() })
  ...
  async function handleStop() {
    await fetch(`/api/platform/agents/${agentId}/stop`, { method: 'POST', headers: getAuthHeaders() });
    const r = await fetch(`/api/platform/agents/${agentId}`, { headers: getAuthHeaders() });
    if (r.ok) setAgent(await r.json());
  }

  async function handleDelete() {
    if (!confirm('Are you sure you want to undeploy this agent?')) return;
    await fetch(`/api/platform/agents/${agentId}`, { method: 'DELETE', headers: getAuthHeaders() });
    router.push('/agents');
  }
