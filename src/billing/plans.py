...
class UsageEnforcer:
    ...
    def record_usage(
        self, tenant_id: str, metric: str, amount: float,
        limit: float | None = None,
    ) -> dict:
        """Record usage for billing. Optionally enforces a limit atomically.

        Returns ``{allowed, total}`` when *limit* is given, else ``{recorded: True}``.
        The INSERT and the limit check run in a single statement so concurrent
        ...
        with self._db.admin() as conn:
            ...
            if limit is not None and limit > 0:
                ...
                total = float(row["amount"]) if row else 0
                return {"allowed": total <= limit, "total": total}
