...
async def _api(method: str, path: str, body: dict | None = None, timeout: float = 120) -> dict:
...
@server.tool()
async def forgeos_approve(request_id: str, reason: str = "Approved via MCP") -> str:
    """Approve a pending HITL request by its ID."""
    return _fmt(await _api("POST", f"/api/approvals/{request_id}/approve", {"reason": reason}))


@server.tool()
async def forgeos_reject(request_id: str, reason: str = "Rejected via MCP") -> str:
    """Reject a pending HITL request by its ID."""
    return _fmt(await _api("POST", f"/api/approvals/{request_id}/reject", {"reason": reason}))
...
async def forgeos_deploy_yaml(manifest_yaml: str) -> str:
    ...
    """
    return _fmt(await _api("POST", "/api/platform/agents/from-yaml", {"yaml_content": manifest_yaml}))
...
async def forgeos_invoke(
    ...
    return _fmt(await _api("POST", f"/api/platform/agents/{agent_id}/invoke", body))
