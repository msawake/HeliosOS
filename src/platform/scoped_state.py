...
class ScopedStateManager:
    ...
    def get(self, scope: StateScope, owner: str, key: str, default: Any = None) -> Any:
        return self._stores[scope].get(owner, {}).get(key, default)

    def set(self, scope: StateScope, owner: str, key: str, value: Any) -> None:
        if owner not in self._stores[scope]:
            self._stores[scope][owner] = {}
        self._stores[scope][owner][key] = value
        self._dirty.add((scope, owner, key))

    def delete(self, scope: StateScope, owner: str, key: str) -> bool:
        bucket = self._stores[scope].get(owner, {})
        if key in bucket:
            del bucket[key]
            self._dirty.add((scope, owner, key))
            return True
    ...
    def list_keys(self, scope: StateScope, owner: str) -> list[str]:
        return list(self._stores[scope].get(owner, {}).keys())

    def get_all(self, scope: StateScope, owner: str) -> dict[str, Any]:
        return dict(self._stores[scope].get(owner, {}))
    ...
    def flush_dirty(self) -> list[tuple[StateScope, str, str, Any]]:
        """Return and clear the dirty set. Each entry is (scope, owner, key, value_or_None)."""
        changes = []
        for scope, owner, key in self._dirty:
            value = self._stores[scope].get(owner, {}).get(key)
            changes.append((scope, owner, key, value))
        self._dirty.clear()
        return changes
    ...
    def load_scope(self, scope: StateScope, owner: str, data: dict[str, Any]) -> None:
        ...
        self._stores[scope][owner] = dict(data)
