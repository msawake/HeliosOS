...
class InMemoryMemoryStore:
    ...
    def _scoped_id(self, agent_id: str) -> str:
        """Prefix agent_id with tenant_id to ensure cross-tenant isolation."""
        if self._tenant_id:
            return f"{self._tenant_id}/{agent_id}"
        return agent_id

    def _agent_store(self, agent_id: str) -> dict[str, MemoryEntry]:
        key = self._scoped_id(agent_id)
...
class FileSystemMemoryStore:
    ...
    def __init__(self, agents_root: str | Path = "agents", tenant_id: str = "") -> None:
    ...
    def _memory_dir(self, agent_id: str) -> Path:
        ...
        if self._tenant_id:
            safe_tenant = re.sub(r"[^a-zA-Z0-9_-]", "_", self._tenant_id)
            return self._root / "tenants" / safe_tenant / safe_id / "memory"
