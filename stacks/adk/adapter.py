...
Helios OS tools as native ADK tools. When the SDK is not available, it falls back
...
- Tool bridge: each Helios OS tool becomes a `ForgeOSAdkTool` (a `BaseTool`
...
- Runner: per-invocation `run_async(user_id, session_id, new_message)` consumes
  the event stream and assembles a Helios OS `AgentResult`. Platform-managed
...
def _sanitize_tool_name(name: str) -> str:
    """Preserve the Helios OS tool name whenever it is already valid; otherwise
...
def _to_genai_schema(input_schema: dict | None) -> Any:
    """Convert a Helios OS tool input_schema (Anthropic-style JSON schema) into a
...
if ADK_AVAILABLE and ADKBaseTool is not None:
    ...
    class ForgeOSAdkTool(ADKBaseTool):
        """Bridges one Helios OS tool into ADK with full schema fidelity.
        ...
        verbatim to the Helios OS tool executor.
        """
        ...
        def __init__(self, *, forgeos_name: str, description: str,
            ...
            super().__init__(
                ...
                description=description or f"Helios OS tool: {forgeos_name}",
            )
...
def _build_adk_tools(tool_executor, agent_def: AgentDefinition, agent_context: dict) -> list:
    """Wrap Helios OS tools as `ForgeOSAdkTool` instances.

    Each declared tool carries the real Helios OS name (sanitized only when
...
class ADKAdapter(AgentStackAdapter):
    ...
    def scaffold_files(self, agent_def: AgentDefinition) -> dict[str, str]:
        ...
        agent_py = textwrap.dedent(f'''\
            ...
            Auto-generated by Helios OS. Run with:
        ...
        tools_py = textwrap.dedent(f'''\
            """
            Helios OS tool wrappers for ADK agent: {agent_def.name}

            At runtime, when the Helios OS platform hosts this agent, tools are
            ...
            # The live wrapper list is injected by Helios OS at runtime.
        ...
        system_prompt_txt = textwrap.dedent(f"""\
            ...
            - Use approved tools via the Helios OS tool executor
